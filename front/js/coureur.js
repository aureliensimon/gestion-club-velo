...
function loadRunner (data) {
    ...
    $('#modifier').on('submit', (event) => {
        ...
        ajaxRequest ('PUT', '../api/coureurs.php/runner/', () => {
            ajaxRequest ('GET', '../api/coureurs.php/runners', loadRunners);
        },'mail=' + $("input[name=mail]").val()+'&nom=' + $("input[name=nom]").val() + '&prenom=' + $("input[name=prenom]").val()+'&num_licence=' + $("input[name=num_licence]").val() + '&date_naissance=' + $("input[name=date_naissance]").val() + '&valide=' + $("input[name=valide]").val() + '&club=' + $("input[name=club]").val() + '&categorie=' + $("input[name=categorie]").val() + '&categorie_categorie_valeur=' + $("input[name=categorie_categorie_valeur]").val() + '&raw_mail=' + $("input[name=raw_mail]").val() );
    });
}
